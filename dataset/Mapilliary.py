...
#############################
	# global variables #
#############################
root_dir          = "./../data/Mapilliary/data/"
for training_set, train_file in zip(["validation", "training/"], ["val.csv", "train.csv"]):
    ...
    np_labels         = os.path.join(root_dir, "labels_CS/")
    train_label_file  = os.path.join(root_dir, train_file) # train file
    ...
    for idx, img in enumerate(os.listdir(img_dir)):
        ...
        gt_name = os.path.join(np_labels, img[:-3])
        ...
        idx_mat = label.astype(np.uint8)
        Image.fromarray(idx_mat, mode='L').save(gt_name)
        csv_file.write("{},{}\n".format(img_name, os.path.join(np_labels, img[:-4]) + ".png"))
