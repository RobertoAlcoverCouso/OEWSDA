...
class FCNs(nn.Module):
    ...
    def __init__(self, pretrained_net, n_class):
        ...
        self.relu    = nn.ReLU(inplace=True)
        self.deconv1 = nn.ConvTranspose2d(512, 512, kernel_size=[4,3], stride=2, padding=1, dilation=1, output_padding=1)
        self.bn1     = nn.BatchNorm2d(512)
        self.deconv2 = nn.ConvTranspose2d(512, 256, kernel_size=3, stride=2, padding=1, dilation=1, output_padding=1)
        self.bn2     = nn.BatchNorm2d(256)
        self.deconv3 = nn.ConvTranspose2d(256, 128, kernel_size=3, stride=2, padding=1, dilation=1, output_padding=1)
    ...
    def forward(self, x):
        ...
        x5 = output['x5'].to(self.deconv1.weight.device)  # size=(N, 512, x.H/32, x.W/32)
        x4 = output['x4'].to(self.deconv1.weight.device)  # size=(N, 512, x.H/16, x.W/16)
        x3 = output['x3'].to(self.deconv1.weight.device)  # size=(N, 256, x.H/8,  x.W/8)
        x2 = output['x2'].to(self.deconv1.weight.device)  # size=(N, 128, x.H/4,  x.W/4)
        x1 = output['x1'].to(self.deconv1.weight.device)  # size=(N, 64, x.H/2,  x.W/2)
